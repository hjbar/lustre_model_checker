...
let rec print_list f sep fmt l =
  match l with
  | [] -> ()
  | [ x ] -> f fmt x
...
let rec print_list_eol f sep fmt l =
  match l with
  | [] -> ()
...
and print_tuple_arg_list fmt e_list =
  match e_list with
  | [] -> assert false
  | [ x ] -> fprintf fmt "%a" print_exp x
  | h :: t -> fprintf fmt "%a,@ %a" print_exp h print_arg_list t
...
and print_const_exp fmt ce_list =
  match ce_list with
  | [] -> assert false
...
let print_node fmt nd =
  fprintf fmt
    "@[node %a(@[%a@]) returns (@[%a@])@\n\
     ...
     @[<v 2>let@ @[%a@]@]@\n\
     tel@]"
    Ident.print nd.tn_name print_var_dec_list nd.tn_input print_var_dec_list
    nd.tn_output print_var_dec_list nd.tn_local
